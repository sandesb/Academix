...
app.post('/repositories', async (req, res) => {
  const { title, source_name, image_url, abstract, matric, project_report_url, project_source_code_url, tech_stack } = req.body;
  ...
  try {
    const result = await pool.query(
      ...
      RETURNING *`,
      [title,  source_name, image_url, abstract, matric, project_report_url, project_source_code_url, tech_stack]
    );
...
app.put('/repositories/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [title, source_name, image_url, abstract, matric, project_report_url, project_source_code_url, tech_stack, id]
    );
